...
where $X(t + \mathcal{T})$ is the matrix of time shifted impulses, $X(t)$ is the observed input and 1 is a vector (1,1,1...1).\\
...
where $\theta_{f}$ and $\theta_{\mathcal{T}}$ represent the parameters of the model $f$ and time shifts $\mathcal{T}$. For simplicity, we assume that the time shift distribution and error distributions are independent. 
