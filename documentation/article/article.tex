...
Let $x(t)$ be a discrete time series. We want to determine the, functional relationship $f:\mathbb{R} \longrightarrow \mathbb{R}$ to some other (target) variable $y$ ob observed data $\{y_i\}_{i=1}^{n}$, that is
$y = f(x(t))$.  The series $x(t)$ must be stationary with a known value for the point at which $f(x(t)) = 0$.  That is, we know at which point the input series has no effect on the output (when it’s off). The size of the support of $f(x(t))$ should be small relative to number of points in the domain (we we will explore this condition further in the 'Limitations' section). We utilise the terminology {\em impulse} for an individual element of the support of $f$.
...
where $X(t + \mathcal{T})$ is the matrix of time shifted impulses, $X(t)$ is the observed input, $1$ is the vector $(1,1,\cdots, 1)\in 
\mathbb{R}^{1 \times k}$, where $k$ is the number of impulses and $\varepsilon$ is a noise term and $\mathcal{T}$ is the set of single impulse time delays ($\tau$'s). The model definition just asummes that the time shifts are applied to the matrix rows and the we reduce by summing up the columns. We assume that $\mathcal{T}$ is not a constant, but rather a random draw from some discrete distribution (e.g. discrete gaussian, poisson etc). For the discrete gaussian kernel $\mathcal{T}\sim T(\mu, \sigma)$ or for the poisson distribution $\mathcal{T}\sim Pois(\lambda)$. Similarly one can model the errors as  $\varepsilon \sim N(\mu, \sigma_{\varepsilon})$. 
...
\begin{enumerate}
    \item [(1)] Initialise the set of parameters $\beta$, $\sigma_{\varepsilon}$ and $\lambda_{\tau}$ ($\theta_{f}$) for the function $f$
    \item [(2)] Find the $\mathcal{T}$ which maximises the likelihood for the given set of parameters:
        ...
        \end{enumerate} 
    \item [(3)] Optimise the parameters $\beta$, $\sigma_{\varepsilon}$ and $\lambda_{\tau}$($\theta_{f}$).

\end{enumerate}
\vspace{5mm} %5mm vertical space
The procedure consists of an inner optimisation to find the set of $\mathcal{T}$ and an outer optimisation to find the set of parameters $\beta$, $\sigma_{\varepsilon}$ and $\lambda_{\tau}$. 
...
In comparison, the algorithm described herein estimates $\beta$ to be $2.09$, much closer to the true value.
